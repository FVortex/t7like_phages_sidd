###function [prob,u,un,Gamma0] = melt_prob_lk(temp,sigma,seq,bc,Gamma,omega)
...
temp    <- 270
N <- 12
melt_prob_lk <- function(temp,sigma,seq,bc,Gamma,omega) {}
[prob,u,un,Gamma0]
#global dna_melt;
 ...
 # if ~isnumeric(seq)
  #  [h seq] = fastaread(seq);
   # seq = nt2int(seq,'ACGTOnly', true);
    ...
    #end
  #end
N = length(seq) #if fasta is read as character vector
  ###N = length(seq);
...
###end
  ...
  ppfd = mmppder(ppf);
  % maximum
  Gamma0 = fzero(@(Gamma) ppval(ppfd,Gamma), [-0.1,0]);
